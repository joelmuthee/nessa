...
(async function() {
  ...
  function whatsappLink(bag) {
    ...
    let msg = enquireBody(bag);
    msg += `\n\n${SHARE_BASE}${encodeURIComponent(bag.id)}`; // OG share page → WA preview card (Tier 2)
  ...
  function showSuspended() {
    ...
    const css = ('@keyframes tlSusFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}'
      ...
      + '#suspendedOverlay .tl-body{font-size:16px;max-width:460px;line-height:1.65;opacity:0.82;margin:0 0 14px;}'
      + '#suspendedOverlay .tl-offer{font-size:16px;max-width:460px;line-height:1.6;margin:0 0 30px;color:#ead7a8;}'
      + '#suspendedOverlay .tl-offer b{color:#fff;font-weight:700;}'
    ...
    o.innerHTML = (
      ...
      + '<p class="tl-offer">Now you can <b>own this shop outright for a one-time Ksh 20,000</b>, no monthly fees. New stock you post on Instagram pulls straight into your shop. Buyers can filter by category and size to find what they want fast, then order on WhatsApp.</p>'
